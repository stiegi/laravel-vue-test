...
defineProps({products: Array, storage: Object});


</script>

<template>
    <Head :title="`Produkte ${storage.name}`"/>

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Produkte in {{ storage.name }}</h2>
        </template>
        <div class="py-12">
